...
public class RCC_CarControllerV3 : MonoBehaviour {
	...
	public bool steeringHelper = false;
...
//		anyWheel.ConfigureVehicleSubsteps(10f, 2, 2);
	...
	void SoundsInitialize (){
		...
		if(useNOS)
			NOSSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "NOS Sound AudioSource", 5, 10, 0.5f, NOSClip, true, false, false);
		if(useNOS || useTurbo)
			blowSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "NOS Blow", 1, 10, 0.5f, null, false, false, false);
		if(useTurbo){
			turboSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "Turbo Sound AudioSource", .1f, .3f, 0, turboClip, true, true, false);
...
//	private bool isDragSet = false;
	void Inputs(){
		...
		//ss	boostInput = Input.GetKey(RCCSettings.boostKB) ? 2.5f : 1f;
...
//				rigid.AddRelativeTorque (Vector3.up * (((steerInput * _gasInput) * 10000f)), ForceMode.Force); 
	...
	void ApplyEngineSound(float input){
		...
		if(engineSoundOff){
			...
			engineSoundOff.volume = (0.1f - _gasInput) - engineSoundIdle.volume;
		...
		if(engineSoundIdle){
			...
			engineSoundIdle.volume = Mathf.Lerp(0.1f, 0f, engineRPM / (maxEngineRPM / 2f));
...
//			}
	    ...
	    void NOS()
    {
        ...
        else
        {
            ...
            if (PlayerCameraNew == null)
            {
                PlayerCameraNew = HHG_GameManager.Instance.VehicleCamera.GetComponent<RCC_Camera>();
                NosEffect = PlayerCameraNew.transform.GetChild(0).gameObject;
            }
            else if (PlayerCameraNew.thisCam.fieldOfView > 60)
            {
                NosEffect.SetActive(false);
                PlayerCameraNew.thisCam.fieldOfView -= 40f * Time.deltaTime;           
                //NitroPartical.SetActive(false);
            }
        ...
        HHG_UiManager.instance.NosFiller[0].fillAmount = (NoS / 100f);
        HHG_UiManager.instance.NosFiller[1].fillAmount = (NoS / 100f);
    }

    private RCC_Camera PlayerCameraNew;
	private GameObject NosEffect;
	...
	void DriftVariables(){
		...
		if (Mathf.Abs(hit.sidewaysSlip) > .25f)
		{
			...
			if (carPhysics)
			{
				carPhysics.UpdateDriftStatus(true);
		...
		else
		{
			...
			if (carPhysics)
			{
				carPhysics.UpdateDriftStatus(false);
...
public	void ResetCarFrombutton (){
		
	if(speed < 20 && !rigid.isKinematic){
